...
exports.run = (client, message) => {
    message.command(false,async () => {
        ...
        const options = require(`../src/reaction-core/ticketMenu`)(message, tDesc)
        ...
        // also, I decided that active tickets could be mapped by Menu's message.id, but it's not the best option (at least for me) so we should change it later I guess
        await client.channels.get(client.config.ticketChannel).sendMenu(ticketMenu).then(msg => client.database.activeTickets.push(msg.id));
